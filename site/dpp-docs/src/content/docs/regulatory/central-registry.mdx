...
Activation of the registry integration is on the longer roadmap, contingent on the registry API being published. The interface is already modelled, so the implementation can land in days rather than months once the upstream API stabilises.
</Aside>
