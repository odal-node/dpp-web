...
Every field carries a disclosure classification drawn from the product group's own definition rather than hard-coded. A request arrives with a credential in the `X-DPP-Credential` header, a W3C Verifiable Credential asserting the holder's role, and the node resolves that role to an audience, then filters the passport to the disclosure classes that audience may see.

The filtering step is a **pure function**: no network, no database. The credential check verifies the signature, the expiry, the issuer's trust status and, where the issuer publishes one, its revocation list. How a node issues credentials of its own, and their limits, is on [Access credentials](/guides/credentials).

For product groups without an adopted delegated act, the software's own model holds a few fields back for credential holders, such as disassembly instructions or substances of concern. Those splits are modelling choices, not law, and they will follow each delegated act once it exists.
...
Verifiable Credentials answer the second and third: they carry a signed issuer assertion, and any platform that can verify them can accept them. The first is not answered yet. The node accepts a credential on its own, without proof that the presenter is its holder, so a copied credential works until it expires or is revoked. That is one reason credentials a node issues itself are capped at 90 days.
