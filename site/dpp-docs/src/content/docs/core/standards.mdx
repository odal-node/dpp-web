...
The eight European DPP system standards — **EN 18216 through EN 18246**, written by CEN/CENELEC JTC 24 — were published in 2026. They govern how any passport is identified, carried on the product, exchanged, stored, access-controlled, and authenticated. Odal tracks them clause by clause in a maintained conformance matrix, and the core's identifier, carrier, API, and authentication semantics align to the published texts. One honest nuance: the standards are *available* but not yet *harmonised* (cited in the Official Journal) — so we build to them today and claim presumption of conformity only when that citation lands.
